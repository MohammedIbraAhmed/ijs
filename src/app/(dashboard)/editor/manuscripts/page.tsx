...
export default function EditorManuscriptsPage() {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (m) =>
          ...
          m.submittedBy?.name.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
  ...
  const getPriorityBadge = (status: string) => {
    ...
    if (status === 'revision_required')
      return { text: 'Action Required', color: 'bg-error-500' };
    return null;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gradient">All Submissions</h1>
          <p className="text-muted-foreground mt-1">
            Manage and review all manuscript submissions
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Select value={typeFilter} onValueChange={setTypeFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="short-communication">
                  Short Communication
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              <p className="text-muted-foreground text-center">
                {manuscripts.length === 0
                  ? 'No manuscripts in the system yet.'
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
                          <span>
                            By {manuscript.submittedBy?.name || 'Unknown'}
                          </span>
                          <span>•</span>
                          ...
                          {manuscript.assignedEditor && (
                            <>
                              <span>•</span>
                              <span>Editor: {manuscript.assignedEditor.name}</span>
                            </>
                          )}
                        </div>
                      </div>
